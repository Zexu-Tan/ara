    ...
    float *z_imag) 
{
    ...
    for (int ptr_col = 0; ptr_col < nr; ptr_col++) {
        //y0
        vfloat32m1_t t_y_real = vle32_v_f32m1(&Y_real[GET_IDX(ptr_col, 0, 0, 1, vl)], vl);
        vfloat32m1_t t_y_imag = vle32_v_f32m1(&Y_imag[GET_IDX(ptr_col, 0, 0, 1, vl)], vl);

        //A0
        vfloat32m1_t A_real = vle32_v_f32m1(&H_real[GET_IDX(ptr_col, 0, 0, nr, vl)], vl);
        vfloat32m1_t A_imag = vle32_v_f32m1(&H_imag[GET_IDX(ptr_col, 0, 0, nr, vl)], vl);
        A_imag = vfneg_v_f32m1(A_imag, vl);

        vcfmac_vv_f32m1_sim(A_real, A_imag, t_y_real, t_y_imag, z0_real, z0_imag, &z0_real, &z0_imag, vl);

        //B0
        vfloat32m1_t B_real = vle32_v_f32m1(&H_real[GET_IDX(ptr_col, 1, 0, nr, vl)], vl);
        vfloat32m1_t B_imag = vle32_v_f32m1(&H_imag[GET_IDX(ptr_col, 1, 0, nr, vl)], vl);
        B_imag = vfneg_v_f32m1(B_imag, vl);

        vcfmac_vv_f32m1_sim(B_real, B_imag, t_y_real, t_y_imag, z1_real, z1_imag, &z1_real, &z1_imag, vl);
        
        //C0
        vfloat32m1_t C_real = vle32_v_f32m1(&H_real[GET_IDX(ptr_col, 2, 0, nr, vl)], vl);
        vfloat32m1_t C_imag = vle32_v_f32m1(&H_imag[GET_IDX(ptr_col, 2, 0, nr, vl)], vl);
        C_imag = vfneg_v_f32m1(C_imag, vl);

        vcfmac_vv_f32m1_sim(C_real, C_imag, t_y_real, t_y_imag, z2_real, z2_imag, &z2_real, &z2_imag, vl);
        
        //D0
        vfloat32m1_t D_real = vle32_v_f32m1(&H_real[GET_IDX(ptr_col, 3, 0, nr, vl)], vl);
        vfloat32m1_t D_imag = vle32_v_f32m1(&H_imag[GET_IDX(ptr_col, 3, 0, nr, vl)], vl);
