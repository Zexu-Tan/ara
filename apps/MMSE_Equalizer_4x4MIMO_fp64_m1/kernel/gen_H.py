...
def emit_array_definition(H, var_name="H"):
    ...
    print(f'double {var_name}[{NT}][{NR}][{N_BEAM}] = {{')
    for t in range(NT):
        print('  {')  # start Nt block
        for r in range(NR):
            row = ", ".join(f"{val:.8f}" for val in H[t, r])
            print(f'    {{ {row} }},')
        print('  },')
    print('};')
...
def main():
    ...
    p.add_argument("--N_BEAM", type=int, default= 1024, help="Third dimension (default 128)")
